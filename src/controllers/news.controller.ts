...
export const createNews: RequestHandler = async (req: Request, res: Response) => {
    const newsService = new NewsService();
    try {
        const { titulo, descripcion } = req.body;
        const imagen = req.file ? req.file.filename : null;
        const fechaPublicacion = new Date();
        const news = await newsService.create({ titulo, descripcion, imagen, fechaPublicacion });
...
export const getNewsById: RequestHandler = async (req: Request, res: Response) => {
    const { id } = req.params;
    const newsService = new NewsService();
    try {
...
export const updateNews: RequestHandler = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const news = await newsService.update(id, { titulo, descripcion, imagen, fechaPublicacion });
